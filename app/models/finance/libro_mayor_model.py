...
class ReglasGastos(Base, AuditMixin):
    ...
    tipo_regla: Mapped[str] = mapped_column(String(20), nullable=False)  # 'CUENTA', 'MIXTA', 'TEXTO'
    cuenta: Mapped[str] = mapped_column(String(15), nullable=True)  # Mapea con cuenta_asociada de SAP
    ...
    filtro_texto: Mapped[str] = mapped_column(String(255), nullable=True)  # Texto que debe incluir
    texto_excluido: Mapped[str] = mapped_column(String(255), nullable=True)  # Texto que NO debe incluir
    ...
    codigo: Mapped[str] = mapped_column(String(50), nullable=False)  # Categoría Ejecutiva 1
    subcodigo: Mapped[str] = mapped_column(String(50), nullable=False)  # Categoría Ejecutiva 2
    nombre_cuenta: Mapped[str] = mapped_column(String(100), nullable=False)  # Nombre Destino para Reporte
    activo: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)

class LibroMayor(Base, AuditMixin):
    __tablename__ = "libro_mayor"
    __table_args__ = (UniqueConstraint("transaccion_id", "linea"), {"schema": "finance"})
